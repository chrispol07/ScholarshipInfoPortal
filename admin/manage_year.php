...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM year_list where id={$_GET['id']}")->fetch_array();
	foreach($qry as $k => $v){
		$$k = $v;
	}
}
			...
			$.ajax({
				...
				success:function(resp){
					if(resp == 1){
						...
						$('#msg').html('<div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> Year already exist.</div>')
						end_load()
					}
				}
			})
