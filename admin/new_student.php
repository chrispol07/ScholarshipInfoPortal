				...
				<div class="row">
					<div class="col-md-6 border-right">
						...
						<div class="form-group">
							...
							<select name="year" id="year" class="form-control form-control-sm select2" required>
								<option value=""></option>
								<?php
								$qry = $conn->query("SELECT * FROM year_list");
								while ($row = $qry->fetch_assoc()) :
								?>
									<option value="<?php echo $row['id'] ?>" <?php echo isset($year) && $year == $row['id'] ? "selected" : "" ?>><?php echo $row['level']." Year" ?></option>
								<?php endwhile; ?>
							</select>
						</div>
						<div class="form-group">
					...
					<div class="col-md-6">
						<div class="form-group">
							<label for="" class="control-label">Org</label>
							<select name="org_id" id="org_id" class="form-control form-control-sm select2" required>
								<option value=""></option>
								<?php
								$orgs = $conn->query("SELECT * FROM org_list");
								while ($row = $orgs->fetch_assoc()) :
								?>
									<option value="<?php echo $row['id'] ?>" <?php echo isset($org_id) && $org_id == $row['id'] ? "selected" : "" ?>><?php echo $row['name'] ?></option>
								<?php endwhile; ?>
							</select>
						</div>
						<div class="form-group">
							<label for="" class="control-label">Status</label>
							<select name="status" id="status_id" class="form-control form-control-sm select2" required>
								<option value=""></option>
								<?php
								$qry = $conn->query("SELECT * FROM status_list");
								while ($row = $qry->fetch_assoc()) :
								?>
									<option value="<?php echo $row['id'] ?>" <?php echo isset($status) && $status == $row['id'] ? "selected" : "" ?>><?php echo $row['status'] ?></option>
								<?php endwhile; ?>
							</select>
						</div>
						<div class="form-group">
		...
		$.ajax({
			...
			success: function(resp) {
				if (resp == 1) {
					...
					$('#email_msg').html("<div class='alert alert-danger'>Email already exist.</div>");
					$('[name="email"]').addClass("border-danger")
					end_load()
				}else if (resp == 3) {
					$('#school_id_msg').html("<div class='alert alert-danger'>ID already exist.</div>");
