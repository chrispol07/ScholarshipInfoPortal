				...
				<div class="row">
					<div class="col-md-4">
						<div class="card card-outline card-info">
							...
							<div class="card-body">
								<form action="" id="manage_criteria">
							...
							<div class="card-footer">
								<div class="d-flex justify-content-end w-100">
									<button class="btn btn-sm btn-primary btn-flat bg-gradient-primary mx-1" form="manage_criteria">Save</button>
									<button class="btn btn-sm btn-flat btn-secondary bg-gradient-secondary mx-1" form="manage_criteria" type="reset">Cancel</button>
								</div>
							</div>
						</div>
					</div>
					<div class="col-md-8">
						<div class="callout callout-info">
							...
							<?php else : ?>
								<div>There's no criteria in the database yet</div>
							<?php endif; ?>
						</div>
					</div>
				</div>
...
<script>
	$(document).ready(function() {
		...
		$('.edit_criteria').click(function() {
			...
			$('#manage_criteria').find("[name='id']").val(criteria[id].id)
			$('#manage_criteria').find("[name='criteria']").val(criteria[id].criteria)
		})
		$('#manage_criteria').on('reset', function() {
			$(this).find('input:hidden').val('')
		})
		$('.delete_criteria').click(function() {
			_conf("Are you sure to delete this criteria?", "delete_criteria", [$(this).attr('data-id')])
		})
		$('#manage_criteria').submit(function() {
			e.preventDefault();
				start_load()
				$('#msg').html('')
				$.ajax({
